...
def main(args, params={}):
    ...
    base_model = utils.load_model(args.base_model_path)
    ...
    exp_name = f'{WANDB_GROUP}_a={config.alpha}_T={config.temp}_sched={args.use_sched}'

    if config.use_wandb:
        wandb.init(project='dla-kws', config=dataclasses.asdict(config), name=exp_name, group=WANDB_GROUP)
        wandb.watch(st_model)

    history = train_distillation(
        st_model, base_model,
        train_loader, val_loader,
        melspec_train, melspec_val,
        opt, config, alpha=config.alpha, temp=config.temp, scheduler=scheduler)

    print('FINAL_METRIC', history['val_metric'][-1])
    save_path = str(save_dir / (exp_name + '.tar'))
    utils.save_model(st_model, save_path)
...
if __name__ == "__main__":
    ...
    paths = (
        ...
        '/home/dpozdeev/dla-kws/save/Base/1105_021522/Base_sched=False.tar',
    )
    ...
    for path in paths:
        for alpha in [0.9, 0.95]:
            for lr in [3e-4, 1e-4]:
                ...
                hist = main(args, params)
                cur_res = hist['val_metric'][-1]
                if cur_res < best_metric:
                    best_metric = cur_res
                    best_params = {
                        'path': path,
                        'alpha': alpha,
